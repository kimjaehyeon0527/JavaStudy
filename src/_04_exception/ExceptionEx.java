...
// => Java는 예외가 발생하면 예외 클래스로부터 객체를 생성하며, 해당 객체는 예외 처리시 사용됨.
...
// try 블록에서 작성한 코드에서 예외가 발생 -> catch 실행 o , finally 실행 o
// 심지어 try, catch 블록에서 return 문을 사용해도 finally는 실행된다. (생략 가능)
...
//        case 3. 예외 처리 코드
        try {
            ...
            System.out.println("배열 인덱싱 중 에러 발생 >> " + e.toString()); //  java.lang.NullPointerException: Cannot invoke "String.length()" because "str" is null
        }
...
//        case 4.
        ...
        try {
            ...
            int number = scanner.nextInt();     // java.lang.InputMismatchException
