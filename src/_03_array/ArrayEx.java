...
// 표준 배열
// 배열
// - 배열에는 저장할 자료형을 선언해주고, 배열 이름과 크기를 선언해야함.
// - 배열의 원소는 모두 같은 타입
...
//        배열 변수 선언
//        - 두 가지 방법이 있으나 관례적으로는 첫 번째 방법을 사용한다.
//        1. 타입[] 변수;
//        2. 타입 변수[];
        int[] arr1;
        int arr2[];
...
//        값 목록으로 배열 변수 선언과 배열 생성
        ...
        System.out.println("intArray : " + intArray);   // 저장된 참조 값 (주소 값)

        intArray[1] = 77;
        System.out.println("intArray[1] : " + intArray[1]);

//        주의. 중괄호로 감싼 목록을 배열 변수에 대입할 때, 배열 변수를 미리 선언한 수에는 값 목록을 대입 불가하다.
        char[] charArray;
//        charArray = {"A", "B", "C"};    // 컴파일 에러
//        배열 변수를 선언하는 시점과, 값 목록 대입하는 시점이 다르다면 "new 타입[]"을 중괄호 앞에 붙여서 대입한다.
        charArray = new char[]{'A', 'B', 'C'};
        System.out.println("charArray[0] : " + charArray[0]);

//         - new 연산자로 배열 선언하고 생성
//        : new 연산자로 배열 처음 생성하면 기본 값으로 초기화 된다.
...
//        - Object src : 원본 배열
//        - int srcPost : 원본 배열의 복사를 시작할 인덱스
//        - Object dest : 새 배열
...
//        binarySearch(arr, val) 메소드 (단, 배열은 정렬된 상태여야 함) - sort 하고 사용.
