...
// - 객체 중복 저장 불가능
// - 하나의 'null'만 저장 가능
// - 수학에서의 집합과 유사한 개념 ( 순서 상관 없고, 중복이 허용되지 않기 때문에 )
// - 요소 순서를 저장하지 않기 때문에 메소드에 인덱스를 매개값으로 갖는 메소드가 없음.

// HashSet
// - Set 컬렉션 중에서 가장 많이 사용되는 컬렉션
// - 동일한 객체 (등등 객체)는 중복 저장하지 않음.
// - 다른 객체더라도 'hashCode()' 메소드의 리턴 값이 같고, 'equals()' 메소드가 'true' 를 리턴하면 동일한 객체라고 판단.

// TreeSet
// - 검색 기능을 강화시킨 컬렉션
// - 이진 트리를 기반으로 한 Set 컬렉션
//  - 이진 트리 : 여러 개의 노드가 트리 형태로 연결된 구조로 root 노드라는 하나의 노드에서 시작해 각 노드에 최대 2개의 노드를 연결 할 수 있음.
